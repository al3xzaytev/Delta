...
def start_level(player_count):
    ...
    while True:
        # Logically, this should only run once at the beginning of every level...
        ...
        # Remove players from player_storage if all players are dead (i.e. level restart)
        if not new_level:
            player_storage.clear()

            # Initializes new players in place of dead ones in level restart
        ...
        # If remaining teammates of the group wins (and moves on to new level),
        # dead players should be revived and original stats brought back.
        elif new_level:
            for players, info in player_list.copy().items():
                if info[1] == "Dead":
                    player_list[players] = [players.name, "Alive"]
                    continue
                else:
                    continue
